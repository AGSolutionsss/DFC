...
const AgenciesSummaryReport = (props) => {
  const componentRef = useRef();
  const [agenciessummary, setSummary] = useState({});
  ...
  useEffect(() => {
    ...
    axios({
      url: baseURL+"/fetch-agencies-report",
      method: "POST",
      data,
      headers: {
  ...
  return (
    <div>
      ...
      <div className="invoice-wrapper">
        <PageTitleBar title="Agencies Summary" match={props.match} />
        <div className="row">
                ...
                <div className="d-flex justify-content-between" style={{fontSize: '16px' }}>
                  ...
                  <div className="address text-center">
                    ...
                    <h2 style={{paddingTop: '5px'}}>
                      <strong>
                        <b style={{fontSize: '20px'}}>AGENCIES SUMMARY</b>
                      </strong>
                    </h2>
                  </div>
                    ...
                    <Table aria-label="simple table" style={{border: '2px solid black', marginTop: '20px'}}>
                      <TableHead>          
                        <TableRow>
                          ...
                          <TableCell style={table_head}>State</TableCell>
                          <TableCell style={table_head}>Mobile</TableCell>
                         </TableRow>
                      </TableHead>
                      <TableBody>
                        {agenciessummary.map((dataSumm, key)=>(
                          <TableRow key={dataSumm.agency_name}>
                            <TableCell style={table_row_count}>{dataSumm.agency_name}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.agency_branch}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.agency_rt_km}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.agency_city}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.agency_state}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.agency_mobile}</TableCell>
                          </TableRow>
                        ))}
                       </TableBody>
                    </Table>
...
export default AgenciesSummaryReport;
