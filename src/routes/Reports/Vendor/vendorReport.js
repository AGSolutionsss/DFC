...
const VendorSummaryReport = (props) => {
  const componentRef = useRef();
  const [vendorsummary, setSummary] = useState({});
  ...
  useEffect(() => {
    ...
    axios({
      url: baseURL+"/fetch-vendor-report",
      method: "POST",
      data,
      headers: {
  ...
  return (
    <div>
      ...
      <div className="invoice-wrapper">
        <PageTitleBar title="Vendors Summary" match={props.match} />
        <div className="row">
                ...
                <div className="d-flex justify-content-between" style={{fontSize: '16px' }}>
                  ...
                  <div className="address text-center">
                    ...
                    <h2 style={{paddingTop: '5px'}}>
                      <strong>
                        <b style={{fontSize: '20px'}}>VENDORS SUMMARY</b>
                      </strong>
                    </h2>
                  </div>
                ...
                <div className="table-responsive">
                  <TableContainer component={Paper}>
                    <Table aria-label="simple table" style={{border: '2px solid black', marginTop: '20px'}}>
                      <TableHead>          
                        <TableRow>
                          <TableCell style={table_head}>Vendor</TableCell>
                          <TableCell style={table_head}>Vendor Type</TableCell>
                          <TableCell style={table_head}>Branch</TableCell>
                          <TableCell style={table_head}>Contact Person</TableCell>
                          <TableCell style={table_head}>Mobile</TableCell>
                         </TableRow>
                      </TableHead>
                      <TableBody>
                        {vendorsummary.map((dataSumm, key)=>(
                          <TableRow key={dataSumm.vendor_name}>
                            <TableCell style={table_row_count}>{dataSumm.vendor_name}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.vendor_type}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.vendor_branch}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.vendor_contact_person}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.vendor_mobile}</TableCell>
                          </TableRow>
                        ))}
                       </TableBody>
                    </Table>
