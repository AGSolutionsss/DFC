...
export default class NewListServiceTypes extends React.Component {
  state = {
    loader: true,
    users: [],
    serviceData: [],
  ...
  getData = () => {
    let result = [];
    axios({
      url: baseURL+"/web-fetch-service-types-list",
      method: "GET",
      headers: {
      ...
      .then((res) => {
        ...
        let response = res.data.serviceTypes;
        let tempRows = [];
        for (let i = 0; i < response.length; i++) {
          
            tempRows.push([
              i + 1,
              response[i]["service_types"],
              response[i]["service_types_status"],
              response[i]["id"],
            ]);
