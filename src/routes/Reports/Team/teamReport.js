...
const TeamSummaryReport = (props) => {
  ...
  const onSubmit = (e) => {
    ...
    setIsButtonDisabled(true)
      axios({
        url: baseURL+"/download-team-report",
        method: "POST",
        data,
        headers: {
          Authorization: `Bearer ${localStorage.getItem("login")}`,
  ...
  return (
    <div>
      ...
      <div className="invoice-wrapper">
        <PageTitleBar title="Team Summary" match={props.match} />
        <div className="row">
                ...
                <div className="d-flex justify-content-between" style={{fontSize: '16px' }}>
                  ...
                  <div className="address text-center">
                    ...
                    <h2 style={{paddingTop: '5px'}}>
                      <strong>
                        <b style={{fontSize: '20px'}}>TEAM SUMMARY</b>
                      </strong>
                    </h2>
                  </div>
                    ...
                    <Table aria-label="simple table" style={{border: '2px solid black', marginTop: '20px'}}>
                      <TableHead>          
                        <TableRow>
                          ...
                          <TableCell style={table_head}>Salary</TableCell>
                          <TableCell style={table_head}>User Type</TableCell>
                          <TableCell style={table_head}>Status</TableCell>
                         </TableRow>
                      </TableHead>
                      <TableBody>
                        {teamsummary.map((dataSumm, key)=>(
                          <TableRow key={dataSumm.full_name}>
                            <TableCell style={table_row_count}>{dataSumm.full_name}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.user_branch}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.user_company}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.mobile}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.email}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.user_address}</TableCell>
                            <TableCell style={table_row_count}>
                              <NumberFormat 
                                ...
                                displayType={'text'}
                                prefix={'₹ '} 
                                value={dataSumm.user_salary}
                              />
                            </TableCell>
                            <TableCell style={table_row_count}>{dataSumm.user_type_id}</TableCell>
                            <TableCell style={table_row_count}>{dataSumm.user_status}</TableCell>
                          </TableRow>
                        ))}
                       </TableBody>
                    </Table>
